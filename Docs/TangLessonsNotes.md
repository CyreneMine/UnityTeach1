...
- P79 排行榜数据逻辑中要注意：教程中只按时间排序的写法可以运行，但业务规则不够完整；更合理的排行榜通常应先按分数降序，再在分数相同时按用时升序。
- P79 中 `List<T>.Sort(Comparison<T>)` 的返回值不是 true/false，而是负数表示第一个对象排前、正数表示第二个对象排前、`0` 表示两者排序上相等。
- P79 中同名成绩覆盖时，新分数更高应同时覆盖分数和时间；分数相同只保留更短时间；新分数更低则忽略，避免把新分数和旧时间拼成错误记录。
- P80 背景音乐中，当前阶段用 `AudioSource` + 简单脚本控制播放、静音和音量是合适的；后续多场景复用时再考虑 `DontDestroyOnLoad` 或统一音频管理器。
- P83 设置面板复用中要注意：BeginScene 专用逻辑不能无条件带到 GameScene，例如关闭设置面板后调用 `BeginPanel.Instance.ShowMe()`；切场景后静态实例还可能保存已销毁对象引用，导致 `MissingReferenceException`。
- P85 坦克基类中，把攻击、防御、血量、移动/旋转速度、死亡表现等公共内容抽到基类，适合后续玩家和敌人复用。
- P86 玩家控制中，教程旧版 `Input.GetAxis` 写法现在仍可用，适合快速练习和理解每帧输入；新版 `Unity Input System` 更适合配置化输入、多设备扩展和后续维护。
- P86 新版输入实践中，使用 `performed/canceled` 回调保存当前 Move / Look 输入向量，再在 `Update()` 中按当前输入驱动玩家移动和旋转。
- P87 小地图中，当前项目使用 UGUI 时不必照搬旧 GUI 写法；`RenderTexture` + 小地图摄像机 + `RawImage` 是更符合当前项目的方案。
- P88 武器和子弹对象中，子弹属于主动移动且需要触发检测的对象，给子弹配置 Rigidbody + Trigger Collider 会比依赖墙体 Rigidbody 更稳定。
- P89-P90 奖励对象中，`OnTriggerEnter` 必须过滤触发者；奖励只应该响应玩家，否则子弹、特效、墙体等对象进入 Trigger 时会导致 `GetComponent<PlayerObj>()` 为空。
- P91 可击毁箱子中，可击毁和不可击毁墙体可以用 Tag 区分，但后续复杂项目更推荐结合 Layer、组件或接口进一步约束伤害对象。
- 当前小项目进度：P91「可击毁箱子」已完成；明天从 P92「固定不动的敌人」继续，目标完成到 P99「实践总结」。
