...
基础阶段已完成到 `Lesson21 / Audio 基础理论`。随后为了正式制作入门小 Demo，额外补完了 `PlayerPrefs`、`UGUI`、`Json 持久化` 三部分前置内容。

前置学习内容包括：物体位移方式、音频导入参数、`AudioSource` / `AudioListener` 组件、`AudioSource` API、麦克风录制与 `AudioClip.GetData()`，以及菜单/设置界面和本地数据保存所需的 UGUI、PlayerPrefs、Json 基础。

当前项目处于“唐老狮入门小项目制作阶段”。实践小项目视频范围已记录为 `P67-P99`，目前已完成到 `P91 / 可击毁箱子`，GameScene 已具备玩家移动、开火、小地图、武器奖励、属性奖励和可击毁箱子等核心互动。下一步进入 `P92 / 固定不动的敌人`，计划继续完成敌人、血条、通关/失败界面、打包和实践总结。旧式 `OnGUI` 只作为了解内容，后续项目优先使用 UGUI。
